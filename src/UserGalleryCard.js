...
import {Card, Button, CardDeck, Col, ButtonGroup, Pagination, Container} from 'react-bootstrap';
...
class UserGalleryCard extends Component {
  ...
  render() {
    ...
    if (this.props.projects[this.props.project].tokens){
      ...
      for (let number = 1; number <= Math.ceil(projectTokens.length/12); number++) {
        items.push(
...
/*
    ...
    */
    ...
    return (
                <div>
                  <CardDeck>
                    {this.props.projects[this.props.project].tokens.map((token,index)=>{
                      if (this.props.projects[this.props.project].tokens.slice((this.state.page-1)*12,this.state.page*12).includes(token)){
                        return (
                          <div key={index}>
                          <Col>
                            ...
                            <Card.Body>
                              ...
                              <ButtonGroup size="sm">
                              <Button variant="light" disabled>#{Number(token)-Number(this.props.project)*1000000}</Button>
